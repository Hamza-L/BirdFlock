...
namespace hva{
    ...
    void NewVulkanApp::loadModels() {
        ...
        float w,h;
        if(vulkanSwapChain->height()>=vulkanSwapChain->width()) {
            w = 1;
            h = (float) vulkanSwapChain->height() / (float) vulkanSwapChain->width();
        } else {
            h = 1;
            w = (float) vulkanSwapChain->width() / (float) vulkanSwapChain->height();
        }
        uboVP.P = glm::ortho(-w,w,-h,h,0.1f,100.0f);
        uboVP.V = glm::lookAt(glm::vec3(0.0f,0.0f,1.0f), glm::vec3(0.0f,0.0f,0.0f), glm::vec3(0.0f,1.0f,0.0f));
        ...
        for (int i = 0; i<MAX_OBJECTS; i++){
            ...
            flock.push_back(std::move(std::make_unique<Bird>(device)));
            flock.back()->h = h;
            flock.back()->w = w;
        }
            ...
            for (int m = 0; m < flock.size(); m++) {
                for (int l = 0; l<flock[m]->objModels.size(); l++) {
                    ...
                    if (TEXTURE) {
                        ...
                        std::array<VkDescriptorSet, 1> descriptorSetGroup = {descriptorSets[imageIndex]};

                        vkCmdBindDescriptorSets(commandBuffers[imageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS,
                                                pipelineLayout, 0, 1, descriptorSetGroup.data(),
                                                0,
                                                nullptr); //we have no dynamic offset yet (only using uniform buffer);
                    }
    ...
    std::vector<Vertex> NewVulkanApp::getNormals(std::vector<Vertex> shape) {
        ...
        for(int i=0; i<shape.size(); i++){
            ...
            v1.colour = glm::vec4(1.0f,1.0f,1.0f,1.0f);
            Vertex v2;
    ...
    void NewVulkanApp::updateModels() {
        ...
        //uboVP.lightPos = glm::vec4((float) xPos * 1.0f, (float) yPos * 1.0f, 2.0f, 1.0f);
